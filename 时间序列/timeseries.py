...
date_index = pd.date_range('2017-01-01', periods=90)
...
def outliers(timeSeries, threshold=3):
    '''异常值处理 采用移动中位数方法'''
    ts_rolling = timeSeries.rolling(window=3, center=True).median()
    ts_dropna = ts_rolling.fillna(method='bfill')
...
def best_diff(timeSeries, maxdiff=8):
    ...
    for i in range(0, maxdiff):
        ...
        pvalue = adfuller(temp['diff'])[1]  # ADF检验返回p值
        p_set[i] = pvalue
...
if __name__ == "__main__":
    ...
    model = ARIMA(ts_diff_2, order=(1, 1))
    ...
    # 移动平均还原
    rol_sum = rol_shift_ts.rolling(window=11).sum()
